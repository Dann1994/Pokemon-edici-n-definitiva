...
| 7.2 | **Captura de Mew — evento "Isla Suprema"** | [mod] `mew_event` | EN CURSO | Diseño → `DESIGN.md`, plan → `PLAN.md`. Decisiones: isla "Isla Suprema", Mew nv. 60, sin encuentros salvajes, música ruta marina (costa) + tema Mansión (bosque), mod propio. **Etapa 1 HECHA** (commit): Mansión 3F — científico oculto que aparece con Liga vencida + Mewtwo capturado + Mewtwo en equipo → se acerca, reconoce a Mewtwo, huye → los "papeles" del 3er piso pasan a contener los 6 documentos de "F." → leerlos todos activa `MOD_MEW_DISCOVERED`. Test 29/29. **Etapa 2 HECHA**: casa del Sr. Fuji (Lavanda) — override de `TEXT_MRFUJISHOUSE_MR_FUJI` con máquina de estados: 1ª charla evasiva (¿F.? · Sudamérica · "una isla que no está en los mapas" · la condición) → `MOD_MEW_FUJI_MYSTERY`; con **150 de Kanto registrados + Mewtwo en el equipo** (`check_dex_owned 150` + verbo `mew_event:party_has MEWTWO`) → 2ª charla: reconoce a Mewtwo, revela "Yo soy F.", entrega el key item **MAPA VIEJO** (`mod.content.items:register`) → `MOD_MEW_OLD_MAP`; luego despedida breve que apunta al puerto. Rama pre-Etapa-1 = diálogo vanilla de Fuji reinsertado literal (dispatch single-winner). Simulación `simulate_stage2.lua` 13/13. **Faltan Etapas 3-5**: marinero de Carmín (mostrar Mapa Viejo → viaje) · mapa Isla Suprema (muelle + cartel "F." + bosque) · estatua + combate `static_battle MEW 60`. |
